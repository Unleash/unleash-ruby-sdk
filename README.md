...
4. Release the gem:
     - Merge the version and changelog changes to the `main` branch.
     - Run the `Publish` GitHub Actions workflow manually.
     - The workflow validates the release with `bundle exec rspec`, `bundle exec rubocop --format simple`, and `bundle exec rake build`.
     - If validation passes, the workflow publishes the gem to [rubygems.org](https://rubygems.org), creates the `vX.Y.Z` git tag, and creates a GitHub release.
